...
public class AppController {
    ...
    public @ResponseBody Iterable<Field> getAllFavorites(
                                                         @PathVariable("userId") String userId) {
        System.out.println("/user/userId/favorites/get");
        ArrayList<Favorite> favorites = new ArrayList<Favorite>();
        ArrayList<Field> favoriteFields = new ArrayList<Field>();

        for (Favorite f : favoriteRepository.getByUserId(userId)) {
            favorites.add(f);
        }
        for (Favorite f : favorites) {
            favoriteFields.add(fieldRepository.getFieldById(f.getFieldId()));
        }
